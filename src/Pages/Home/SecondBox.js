...
import ConfirmationPopup3 from "../../components/confirmationPopup3";
...
const SecondBox = ({
  ...
  const [penality, set_penality] = useState("");
  ...
  async function test() {
    const web3 = new Web3(
      new Web3.providers.HttpProvider("https://bsc.publicnode.com	")
    );
    ...
    let penality = await contract.methods.penality().call();
    let totalbusiness = await contract.methods.totalbusiness().call();
    set_tvl(totalbusiness/10**9)
    set_min_withdraw(minimum_withdraw)
    set_penality(penality)
...
console.log(" object rew "+All_investments_ForReward);
  ...
  async function stake() {
    ...
    if (stakeAmount <minimum_investment) {
      alert("you can't stake less than "+minimum_investment+ " TYON");
      return;
    }
  ...
  function ClaimReward() {
    ...
    if(Number(minimum_withdraw)>Number(totalReward) )
    {
      alert("you can't withdraw less than "+minimum_withdraw/10**9 +" TYON");
      return;
    }
      ...
      <div className="body-bottom flex flex-col w-full">
        ...
        <div className={`expend-detail flex flex-col ${expend ? "show" : ""}`}>
          <div className="detail-item flex items-center justify-between">
            <div className="lbl-side">TVL:</div>
          ...
          <div className="detail-item flex items-center justify-between">
            ...
            <div className="val-side">
              <a
                href={"https://bscscan.com/address/" + stake2_address}
                target="_blank"
      ...
      {selectedTab2 === "Stake" ? (
          ...
          <div className="body-meta flex flex-col justify-between h-full">
            ...
            >
              {!isLoading_stake &&
                !isLoading_app &&
                !isSuccess_app &&
                !stakeSuccess && <div>Stake/Approve</div>}
        ...
        >
          ...
          <div className="body-meta flex flex-col justify-between h-full">
            <div className="flex flex-col">
              <div className="info-list flex flex-col">
                <div className="info-item flex items-center justify-between">
                  ...
                  <h1 className="item-lbl text-red-600">{penality/10**9} %</h1>
                </div>
              </div>
              <div className="input-form flex flex-col">
                <div className="input-field flex flex-col">
            ...
            {selectedAmount? (selectedAmount[7] ?(
              <button className="btn-stack button" onClick={(e) => setOpen3(true) }>Immediate Unstake</button>
        ...
        <ConfirmationPopup setOpen={setOpen} unstaking={unstaking} unstaking_req={unstaking_req} penality={penality/10**9}/>
      </Modal>

      <Modal3 open3={open3} onClose={() => setOpen3(false)}>
        <ConfirmationPopup3 setOpen3={setOpen3} unstaking={unstaking} penality={penality/10**9}/>
      </Modal3>
{/* 
<Modal2 open2={open2} onClose={() => setOpen2(false)}>
        <ConfirmationPopup2 setOpen2={setOpen2} unstaking={unstaking}/>
      </Modal2> */}
    </div>
  );
};
